...
internal static class OverlayManager
{
    ...
    /// <summary>
    /// Tells the page whether it is currently clickable. The multiplayer menu shows itself only
    /// in that state — a modal the player cannot click would just be in the way.
    /// </summary>
    private static void PublishMenuVisibility() =>
        PostMessage("menu", new { visible = _interactiveMode });
